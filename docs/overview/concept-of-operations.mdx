...
Most missions create a scaffold plan built around a notional DSN schedule. To do so, a DSN schedule can be input to an Aerie simulation, translated to a resource. Various scheduling goals can be written to place telecom activities into the viewing periods that satisfy selection criteria. Engineering calibration and turns can be placed relative to these telecom activities.
