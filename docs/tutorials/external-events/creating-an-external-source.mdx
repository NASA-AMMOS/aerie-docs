...
This particular format applies to external sources being uploaded via the user interface. _If_ uploading by other means
(like the CLI, or manually uploading to the `/uploadExternalSource` endpoint), make sure to include in the source header
an additional field that specifies the derivation group. For example:
