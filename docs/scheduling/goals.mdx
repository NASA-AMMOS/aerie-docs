...
- `activityTemplate` - The description of the activity to insert after each activity identified by `forEach`. This can be an `ActivityTemplate` object but also an [arrow function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions) whose parameter would be an `ActivityInstance` object and its return would be an `ActivityTemplate`. This allows to define the content of the `ActivityTemplate` with components of the anchor activity.
