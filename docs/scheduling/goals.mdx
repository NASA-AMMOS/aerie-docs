...
In this final example, the `forEach` condition a defines a `Window` that specifies the time instant at which the goal has to create an instance of the activity template.
```ts
