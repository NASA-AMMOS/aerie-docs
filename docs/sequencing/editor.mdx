...
The landing page lists all sequences available at this venue in the left panel. Upon click users can view the contents of sequences in the right hand side editor in a "read only" mode. To edit an idividual sequence click the pencil icon for the sequence on the left hand side panel as shown in the image below. 
...
Aerie editor allows authoring and viewing sequences in a user friendly format which is more compact, easier to read and write compared to the seqJSON. However seqJSON is the exchange format that should be used to exchange sequences with upstream and downstream tools. While creating a new sequence, users can import a seqJSON using the in (``` Choose File ```)  button available at the new sequence the left pannel. After authoring a sequence, it can be downloaded to the your local machine using the (``` Download ```) button at the upper right corner of the seqJSON view panel at the bottom. 

:::note

Currently Aerie sequence editor only supports outputting **sequences** in the seqJSON format. For immediate and hardware commands, users can post process the output seqJSON until that feature can be supported natively. 

:::

## Typing commands

As explained earlier authoring a sequence simply means concatanating commands in the steps array. Example below illustrates how a simple sequence with a few commands and various time tags looks like. 
...
- If command has arguments type opening and closing brackets as follows  ``` ({ }) ``` , and then list all args comma seperetad in the ``` ({ name: value, name:value }) ``` format. 
- Editor can autocomplete argument names with the ``` control + Space ``` shortcut. 
- For the enumareted args, ``` control + Space ``` can be used to select one of the defined enum values. 
- All commands must be comma seperated. 
