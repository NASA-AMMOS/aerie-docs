...
X-Range plots can be shown as line plots through the corresponding option in the layer settings menu. Use this option when you would like to visualize a discrete resource (Bananation's `producer` resource for example) as a line plot.
:::
